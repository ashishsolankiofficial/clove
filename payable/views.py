...
class BetView(APIView):
    ...
    def get(self, request, ext_id):
        office_bet = OfficeBet.objects.get(ext_id=ext_id)
        serializer = BetSerializer(office_bet)
        return Response(serializer.data)

    def put(self, request, ext_id, format=None):
        office_bet = get_object_or_404(OfficeBet, ext_id=ext_id)
        request.data['office'] = User.objects.get(ext_id=request.user.ext_id).office.ext_id
        serializer = BetSerializer(office_bet, data=request.data, context={'request': request})
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
